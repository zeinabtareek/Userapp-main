  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: GetBuilder<UserController>(builder: (userController){
        return CustomBody(
          appBar: CustomAppBar(title: Strings.makeYourProfileToEarnPoint.tr,showBackButton: false,centerTitle: true,),
          body: Padding(
            ...
            child: SingleChildScrollView(
              child: Column(children:  [
                        ...
                        child: Column(
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                Column(
                                  ...
                                  children: [
                                    Row(children: [
                                      Text("${Strings.level.tr} : ${ userController.userModel?.userLevelModel?.currentLevel}",style: textBold.copyWith(color: Theme.of(context).hintColor,fontSize: Dimensions.fontSizeSmall)),
                                    ...
                                    Row(
                                      children: [
                                        Text('${Strings.yourRating.tr} :'.tr,
                                          style:K.hintSmallTextStyle,),                                        const SizedBox(height: Dimensions.paddingSizeExtraSmall,),
                                        Text('5'.tr,
                            ...
                            Row(
                              mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                              children: [
                                _buildColumnItem(Strings.totalRide,'242',context),
                                Container(width: 1,height: 40,color: Theme.of(context).primaryColor,),
                                _buildColumnItem(Strings.totalPoint,'242',context)
                              ],
                            )
                          ],
                        ),
                ...
                ProfileMenuItem(title: Strings.profile.tr, icon: Images.profileProfile,
                  onTap: ()=>Get.to(()=>const EditProfileScreen()),
                ),
                ProfileMenuItem(title: Strings.myOffer.tr, icon: Images.profileMyOrder,
                    onTap:()=>Get.to(()=>const OfferScreen())
                ),
                ProfileMenuItem(title: Strings.message.tr, icon: Images.profileMessage,
                  onTap: ()=>Get.to(const MessageListScreen()),
                ),
                ProfileMenuItem(title: Strings.myWallet, icon: Images.profileMyWallet,
                  onTap: ()=>Get.to(()=>const WalletScreen()),
                ),
                ProfileMenuItem(title: Strings.myTrips.tr, icon: Images.profileMyTrip,
                  onTap: ()=>Get.to(()=>  HistoryScreen(fromPage: Strings.profile,)),
                ),

                ProfileMenuItem(title: Strings.helpSupport.tr, icon: Images.profileHelpSupport,
                  onTap: ()=>Get.to(const HelpAndSupportScreen()),
                ),
                ProfileMenuItem(title: Strings.settings.tr, icon: Images.profileSetting,
                  onTap: ()=>Get.to(const SettingScreen()),
                ),
                ProfileMenuItem(title: Strings.logout.tr, icon: Images.profileLogout,divider: false,
                  onTap: (){
                    showDialog(context: context, builder: (_){
                      return ConfirmationDialog(icon: Images.profileLogout,
                        title: Strings.logout.tr,
                        description: Strings.doYouWantToLogOutThisAccount.tr, onYesPressed: (){
