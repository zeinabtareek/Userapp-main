...
class HtmlViewerScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return
      ...
      body: CustomBody(
        appBar: CustomAppBar(
            title: isPolicy ? 'privacy_policy'.tr : 'terms_and_condition'.tr,
            showBackButton: true),
        body:
    ...
    return Column(
              children: [
                ...
                // Text(isPolicy ? 'privacy_policy'.tr : 'terms_and_condition'.tr ),
                K.sizedBoxH0,
                (helpAndSupportController.model.data?.terms?.value == null ||
      helpAndSupportController.model.data?.terms?.value == '' ||helpAndSupportController.model.data!.policy!.value == null ||
      helpAndSupportController.model.data?.policy?.value == '')?
      customNoDataWidget():
      HtmlWidget(isPolicy
      ? helpAndSupportController.model.data?.terms?.value ?? '':
      helpAndSupportController.model.data?.policy?.value ?? '')
                ///TODO zeinab don't forget to check this if it worked or not
